...
TEST(FIOEncryption, checkEncrypt) {
    // test derived from
    // https://github.com/fioprotocol/fiojs/blob/master/src/tests/encryption-check.test.ts
    const Data secret =
        parse_hex("02332627b9325cb70510a70f0f6be4bcb008fbbc7893ca51dedf5bf46aa740c0fc9d3fbd737d09a3"
                  "c4046d221f4f1a323f515332c3fef46e7f075db561b1a2c9");
    ...
    EXPECT_EQ(hex(result), "f300888ca4f512cebdc0020ff0f7224c7f896315e90e172bed65d005138f224da7301d5"
                           "563614e3955750e4480aabf7753f44b4975308aeb8e23c31e114962ab");
}
...
TEST(FIOEncryption, checkDecrypt) {
    ...
    const Data encrypted =
        parse_hex("f300888ca4f512cebdc0020ff0f7224c7f896315e90e172bed65d005138f224da7301d5563614e39"
                  "55750e4480aabf7753f44b4975308aeb8e23c31e114962ab");
...
TEST(FIOEncryption, checkDecryptInvalidMessageHMAC) {
    ...
    const Data encrypted =
        parse_hex("f300888ca4f512cebdc0020ff0f7224c7f896315e90e172bed65d005138f224da7301d5563614e39"
                  "55750e4480aabf7753f44b4975308aeb8e23c31e114962ab00");
    try {
...
TEST(FIOEncryption, getSharedSecret) {
    ...
    // https://github.com/fioprotocol/fiojs/blob/master/src/tests/encryption-fio.test.ts See also
    // https://github.com/fioprotocol/fiojs/blob/master/docs/message_encryption.md
    {
        const PrivateKey privateKey(
            parse_hex("2bd806c97f0e00af1a1fc3328fa763a9269723c8db8fac4f93af71db186d6e90"));
        ...
        EXPECT_EQ(hex(secret), "a71b4ec5a9577926a1d2aa1d9d99327fd3b68f6a1ea597200a0d890bd3331df300a"
                               "2d49fec0b2b3e6969ce9263c5d6cf47c191c1ef149373ecc9f0d98116b598");
    }
    {
        const PrivateKey privateKey(
            parse_hex("81b637d8fcd2c6da6359e6963113a1170de795e4b725b84d1e0b4cfd9ec58ce9"));
        const PublicKey publicKey(
    ...
    }
    {
        ...
        const PublicKey publicKey(
            parse_hex("03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5bd"),
            TWPublicKeyTypeSECP256k1);
        Data secret = Encryption::getSharedSecret(privateKey, publicKey);
        EXPECT_EQ(secret.size(), 64);
        EXPECT_EQ(hex(secret), "3f0840df1912e24d85f39008a56550c31403e096fce7fa9d7886fab8e5c2ceb66b4"
                               "139c8f4f4172fd9f455e76c2e8913a3d734f51a1951090ce9ec660671957d");
    }
}
...
TEST(FIOEncryption, encryptAndEncode) {
    ...
    // https://github.com/fioprotocol/fiojs/blob/master/src/tests/encryption-fio.test.ts
    {
        ...
        EXPECT_EQ(
            hex(encrypted),
            "f300888ca4f512cebdc0020ff0f7224c0db2984c4ad9afb12629f01a8c6a76328bbde17405655dc4e3cb30"
            "dad272996fb1dea8e662e640be193e25d41147a904c571b664a7381ab41ef062448ac1e205");
        const string encoded = Encryption::encode(encrypted);
        EXPECT_EQ(encoded,
                  "8wCIjKT1Es69wAIP8PciTA2ymExK2a+"
...
TEST(FIOEncryption, encryptEncodeDecodeDecrypt) {
    const PrivateKey privateKeyAlice(
        parse_hex("2bd806c97f0e00af1a1fc3328fa763a9269723c8db8fac4f93af71db186d6e90"));
    ...
    const PrivateKey privateKeyBob(
        parse_hex("81b637d8fcd2c6da6359e6963113a1170de795e4b725b84d1e0b4cfd9ec58ce9"));
    ...
    EXPECT_EQ(hex(encrypted),
              "f300888ca4f512cebdc0020ff0f7224c0db2984c4ad9afb12629f01a8c6a76328bbde17405655dc4e3cb"
              "30dad272996fb1dea8e662e640be193e25d41147a904c571b664a7381ab41ef062448ac1e205");
    const string encoded = Encryption::encode(encrypted);
    EXPECT_EQ(encoded,
              "8wCIjKT1Es69wAIP8PciTA2ymExK2a+"
              "xJinwGoxqdjKLveF0BWVdxOPLMNrScplvsd6o5mLmQL4ZPiXUEUepBMVxtmSnOBq0HvBiRIrB4gU=");
