...
const PrivateKey privKeyBA =
    PrivateKey(parse_hex("ba0828d5734b65e3bcc2c51c93dfc26dd71bd666cc0273adee77d73d9a322035"));
...
TEST(FIOTransactionBuilder, RegisterFioAddressGeneric) {
    ...
    input.mutable_action()->mutable_register_fio_address_message()->set_fio_address(
        "adam@fiotestnet");
    input.mutable_action()->mutable_register_fio_address_message()->set_owner_fio_public_key(
        addr6M.string());
    ...
    EXPECT_EQ(
        R"({"compression":"none","packed_context_free_data":"","packed_trx":"","signatures":["SIG_K1_K19ugLriG3ApYgjJCRDsy21p9xgsjbDtqBuZrmAEix9XYzndR1kNbJ6fXCngMJMAhxUHfwHAsPnh58otXiJZkazaM1EkS5"]})",
        json);
}
...
TEST(FIOTransactionBuilder, RegisterFioAddress) {
    ...
    string t = TransactionBuilder::createRegisterFioAddress(
        addr6M, privKeyBA, "adam@fiotestnet", chainParams, fee, "rewards@wallet", 1579784511);
...
TEST(FIOTransactionBuilder, AddPubAddress) {
    ...
    string t = TransactionBuilder::createAddPubAddress(
        addr6M, privKeyBA, "adam@fiotestnet",
    ...
    EXPECT_EQ(
        R"({"compression":"none","packed_context_free_data":"","packed_trx":"","signatures":["SIG_K1_K3zimaMKU8cBhVRPw46KM2u7uQWaAKXrnoeYZ7MEbp6sVJcDQmQR2RtdavpUPwkAnYUkd8NqLun8H48tcxZBcTUgkiPGVJ"]})",
        t);
}
...
TEST(FIOTransactionBuilder, Transfer) {
    ...
    string t = TransactionBuilder::createTransfer(addr6M, privKeyBA, payee, amount, chainParams,
                                                  fee, "rewards@wallet", 1579790000);

    EXPECT_EQ(
        R"({"compression":"none","packed_context_free_data":"","packed_trx":"","signatures":["SIG_K1_K9VRCnvaTYN7vgcoVKVXgyJTdKUGV8hLXgFLoEbvqAcFxy7DXQ1rSnAfEuabi4ATkgmvnpaSBdVFN7TBtM1wrbZYqeJQw9"]})",
        t);
}
...
TEST(FIOTransactionBuilder, RenewFioAddress) {
    ...
    string t = TransactionBuilder::createRenewFioAddress(
        addr6M, privKeyBA, "nick@fiotestnet", chainParams, fee, "rewards@wallet", 1579785000);

    EXPECT_EQ(
        R"({"compression":"none","packed_context_free_data":"","packed_trx":"289b295ec99b904215ff0000000001003056372503a85b80b1ba2919aea6ba01102b2f46fca756b200000000a8ed32322f0f6e69636b4066696f746573746e6574005ed0b200000000102b2f46fca756b20e726577617264734077616c6c657400","signatures":["SIG_K1_Jxz7oCJ7Z4ECsxqb2utqBcyP3zPQCeQCBws9wWQjyptUKoWVk2AyCVEqtdMHJwqtLniio5Z7npMnaZB8E4pa2G75P9uGkb"]})",
        t);
}
...
TEST(FIOTransactionBuilder, NewFundsRequest) {
    {
        ...
        const Data iv =
            parse_hex("000102030405060708090a0b0c0d0e0f"); // use fixed iv for testability
        string t = TransactionBuilder::createNewFundsRequest(
            Address("FIO5NMm9Vf3NjYFnhoc7yxTCrLW963KPUCzeMGv3SJ6zR3GMez4ub"), privKeyBA,
            "tag@fiotestnet", "FIO7iYHtDhs45smFgSqLyJ6Zi4D3YG8K5bZGyxmshLCDXXBPbbmJN",
            "dapixbp@fiotestnet", "14R4wEsGT58chmqo7nB65Dy4je6TiijDWc", "1", "BTC", "payment", "",
    ...
    string t = TransactionBuilder::createNewFundsRequest(
        addr6M, privKeyBA, "mario@fiotestnet",
        "FIO5kJKNHwctcfUM5XZyiWSqSTM5HTzznJP9F3ZdbhaQAHEVq575o", "alice@fiotestnet",
        "bc1qvy4074rggkdr2pzw5vpnn62eg0smzlxwp70d7v", "5", "BTC", "Memo", "Hash",
        "https://trustwallet.com", chainParams, fee, "rewards@wallet", 1579785000, iv);
...
TEST(FIOTransaction, ActionRegisterFioAddressInternal) {
    RegisterFioAddressData radata("adam@fiotestnet", addr6M.string(), 5000000000, "rewards@wallet",
                                  "qdfejz2a5wpl");
    Data ser1;
    radata.serialize(ser1);
    EXPECT_EQ(hex(parse_hex("0F6164616D4066696F746573746E65743546494F366D31664D645470526B52426E6564"
                            "765973685843784C4669433573755255384B44667838787874587032686E7478706E66"
                            "00F2052A01000000102B2F46FCA756B20E726577617264734077616C6C6574")),
              hex(ser1));
    ...
    EXPECT_EQ("003056372503a85b0000c6eaa66498ba0"
              "1102b2f46fca756b200000000a8ed3232"
              "65"
              "0f6164616d4066696f746573746e65743546494f366d31664d645470526b52426e656476597368584378"
              "4c4669433573755255384b44667838787874587032686e7478706e6600f2052a01000000102b2f46fca7"
              "56b20e726577617264734077616c6c657400",
              hex(ser2));
    ...
    Data ser3;
    tx.serialize(ser3);
    EXPECT_EQ("3f99295ec99b904215ff0000000001"
...
TEST(FIOTransaction, ActionAddPubAddressInternal) {
    ...
    EXPECT_EQ("0f6164616d4066696f746573746e65740303425443034254432a626331717679343037347267676b6472"
              "32707a773576706e6e3632656730736d7a6c787770373064377603455448034554482a30786365356342"
              "366339324461333762624261393142643430443443394434443732344133613846353103424e4203424e"
              "422a626e6231747333646735346170776c76723968757076326e306a366534367135347a6e6e75736a6b"
              "39730000000000000000102b2f46fca756b20e726577617264734077616c6c6574",
              hex(ser1));
    ...
    EXPECT_EQ("003056372503a85b0000c6eaa66452320"
              "1102b2f46fca756b200000000a8ed3232"
              "c901"
              "0f6164616d4066696f746573746e65740303425443034254432a626331717679343037347267676b6472"
              "32707a773576706e6e3632656730736d7a6c787770373064377603455448034554482a30786365356342"
              "366339324461333762624261393142643430443443394434443732344133613846353103424e4203424e"
              "422a626e6231747333646735346170776c76723968757076326e306a366534367135347a6e6e75736a6b"
              "39730000000000000000102b2f46fca756b20e726577617264734077616c6c657400",
              hex(ser2));
    ...
    Data ser3;
    tx.serialize(ser3);
    EXPECT_EQ("15c2285e2d2d23622eff0000000001"
...
TEST(FIONewFundsContent, serialize) {
    {
        NewFundsContent newFunds{"bc1qvy4074rggkdr2pzw5vpnn62eg0smzlxwp70d7v",
                                 "10",
                                 "BTC",
                                 "BTC",
                                 "Memo",
        ...
        EXPECT_EQ(hex(ser), "2a626331717679343037347267676b647232707a773576706e6e3632656730736d7a6c"
                            "78777037306437760231300342544303425443044d656d6f0448617368176874747073"
                            "3a2f2f747275737477616c6c65742e636f6d0000000000");
    }
    {
        // empty struct
    ...
    {
        // test from
        // https://github.com/fioprotocol/fiojs/blob/master/src/tests/encryption-fio.test.ts
...
TEST(FIONewFundsContent, deserialize) {
    {
        const Data ser = parse_hex("2a626331717679343037347267676b647232707a773576706e6e36326567307"
                                   "36d7a6c78777037306437760231300342544303425443044d656d6f04486173"
                                   "681768747470733a2f2f747275737477616c6c65742e636f6d0000000000");
        size_t index = 0;
...
void createTxWithChainParam(const ChainParams& paramIn, ChainParams& paramOut) {
    string tx = TransactionBuilder::createAddPubAddress(
        addr6M, privKeyBA, "adam@fiotestnet",
        {{"BTC", "bc1qvy4074rggkdr2pzw5vpnn62eg0smzlxwp70d7v"}}, paramIn, 0, "rewards@wallet",
        1579729429);
...
TEST(FIOTransactionBuilder, encodeString) {
    ...
    {
        Data data;
        const string text = "0123456789012345678901234567890123456789012345678901234567890123456789"
