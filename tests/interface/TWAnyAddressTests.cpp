...
TEST(AnyAddress, InvalidString) {
    ...
    auto ethAaddress =
        WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeEthereum));
...
TEST(AnyAddress, Data) {
    ...
    // bnb address key hash
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeBinance));
        auto keyHash = WRAPD(TWAnyAddressData(addr.get()));
    ...
    // segwit witness program
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeBitcoin));
    ...
    // base58 dogecoin
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeDogecoin));
        auto keyHash = WRAPD(TWAnyAddressData(addr.get()));
    ...
    // decred
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeDecred));
        auto keyHash = WRAPD(TWAnyAddressData(addr.get()));
    ...
    // zilliqa
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeZilliqa));

        auto expectedKeyHash = "F9dad33332CF77E783B3452aE982c85effCa6DDf";
        auto keyHash = WRAPD(TWAnyAddressData(addr.get()));
        auto checksumed = WRAPS(
            TWStringCreateWithRawBytes(TWDataBytes(keyHash.get()), strnlen(expectedKeyHash, 40)));
    ...
    // kusama
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeKusama));
        auto pubkey = WRAPD(TWAnyAddressData(addr.get()));
    ...
    // polkadot
    {
        ...
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypePolkadot));
        auto pubkey = WRAPD(TWAnyAddressData(addr.get()));
        assertHexEqual(pubkey, "fab55af5eaaa1a284b615946dc3dd403c44ce3448c15884d61b2dcba3ddb245a");
    }
    // cardano
    {
        auto string = STRING("addr1s3xuxwfetyfe7q9u3rfn6je9stlvcgmj8rezd87qjjegdtxm3y3f2mgtn87mrny9"
                             "r77gm09h6ecslh3gmarrvrp9n4yzmdnecfxyu59jz29g8j");
        auto addr =
            WRAP(TWAnyAddress, TWAnyAddressCreateWithString(string.get(), TWCoinTypeCardano));
        auto pubkey = WRAPD(TWAnyAddressData(addr.get()));
        assertHexEqual(pubkey,
                       "0104204dc3393959139f00bc88d33d4b2582fecc237238f2269fc094b286acdb892295206d0"
                       "b99fdb1cc851fbc8dbcb7d6710fde28df46360c259d482db679c24c4e50b2");
    }
    // neo
    {
