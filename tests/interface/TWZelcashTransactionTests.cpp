...
TEST(ZelcashTransaction, Encode) {
    ...
    auto sighash =
        transaction.getSignatureHash(scriptCode, 0, TWBitcoinSigHashTypeAll, 0x02faf080, BASE);
...
TEST(ZelcashTransaction, Signing) {
    ...
    // Sign
    auto result = Bitcoin::TransactionSigner<Zcash::Transaction, Zcash::TransactionBuilder>(
                      std::move(input), plan)
                      .sign();
    ...
    ASSERT_EQ(hex(serialized),
              ...
              "00000000"
              "6a473044022002f181eeeab108a94a99a5aa0f4aaaf2236f8ea8c0db39ebfdcff8511f55df2102206c67"
              "f325099ca5b0a82e7d5d2b5b600cf4e3c8aec90ae7655b89e14282319a6a012103eeb334e219744f26ce"
              "6bffbdc2122bc13060606a2f696a5891fb101a82c80551"
              "ffffffff"
              "01"
              "9874a40800000000"
