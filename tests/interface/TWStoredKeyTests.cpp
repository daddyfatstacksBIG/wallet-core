...
struct TWStoredKey* _Nonnull createAStoredKey(TWCoinType coin, TWData* password) {
    const auto mnemonic = WRAPS(TWStringCreateWithUTF8Bytes(
...
struct TWStoredKey* _Nonnull createDefaultStoredKey() {
    ...
    const auto password = WRAPD(TWDataCreateWithBytes(
        reinterpret_cast<const uint8_t*>(TWStringUTF8Bytes(passwordString.get())),
        TWStringSize(passwordString.get())));
...
TEST(TWStoredKey, loadPBKDF2Key) {
    const auto filename =
        WRAPS(TWStringCreateWithUTF8Bytes((TESTS_ROOT + "/Keystore/Data/pbkdf2.json").c_str()));
...
TEST(TWStoredKey, loadNonexistent) {
    const auto filenameInvalid =
        WRAPS(TWStringCreateWithUTF8Bytes((TESTS_ROOT + "_NO_/_SUCH_/_FILE_").c_str()));
...
TEST(TWStoredKey, importPrivateKey) {
    ...
    const auto privateKey =
        WRAPD(TWDataCreateWithHexString(TWStringCreateWithUTF8Bytes(privateKeyHex)));
    const auto name = WRAPS(TWStringCreateWithUTF8Bytes("name"));
    const auto passwordString = WRAPS(TWStringCreateWithUTF8Bytes("password"));
    const auto password = WRAPD(TWDataCreateWithBytes(
        reinterpret_cast<const uint8_t*>(TWStringUTF8Bytes(passwordString.get())),
    ...
    const auto key =
        TWStoredKeyImportPrivateKey(privateKey.get(), name.get(), password.get(), coin);
    const auto privateKey2 = WRAPD(TWStoredKeyDecryptPrivateKey(key, password.get()));
    EXPECT_EQ(hex(data(TWDataBytes(privateKey2.get()), TWDataSize(privateKey2.get()))),
              privateKeyHex);
...
TEST(TWStoredKey, importHDWallet) {
    ...
    // invalid mnemonic
    const auto mnemonicInvalid =
        WRAPS(TWStringCreateWithUTF8Bytes("_THIS_IS_AN_INVALID_MNEMONIC_"));
    EXPECT_EQ(TWStoredKeyImportHDWallet(mnemonicInvalid.get(), name.get(), password.get(), coin),
              nullptr);
...
TEST(TWStoredKey, addressAddRemove) {
    ...
    EXPECT_EQ(string(TWStringUTF8Bytes(accountAddress.get())),
    ...
    const auto extPubKeyAdd = "zpub6qbsWdbcKW9sC6shTKK4VEhfWvDCoWpfLnnVfYKHLHt31wKYUwH3aFDz4WLjZvjH"
                              "Z5W4qVEyk37cRwzTbfrrT1Gnu8SgXawASnkdQ994atn";

    TWStoredKeyAddAccount(key, WRAPS(TWStringCreateWithUTF8Bytes(addressAdd)).get(),
...
TEST(TWStoredKey, importJsonInvalid) {
    ...
    auto json = WRAPD(
        TWDataCreateWithBytes((const uint8_t*)jsonInvalidStr.c_str(), jsonInvalidStr.length()));
...
TEST(TWStoredKey, importInvalidKey) {
    ...
    auto password = WRAPD(TWDataCreateWithBytes(
        reinterpret_cast<const uint8_t*>(TWStringUTF8Bytes(name.get())), TWStringSize(name.get())));

    auto eth =
        TWStoredKeyImportPrivateKey(data.get(), name.get(), password.get(), TWCoinTypeEthereum);
    auto ont =
        TWStoredKeyImportPrivateKey(data.get(), name.get(), password.get(), TWCoinTypeOntology);
    auto tezos =
        TWStoredKeyImportPrivateKey(data.get(), name.get(), password.get(), TWCoinTypeTezos);
...
TEST(TWStoredKey, getWalletPasswordInvalid) {
    ...
    const auto passwordInvalid = WRAPD(TWDataCreateWithBytes(
        reinterpret_cast<const uint8_t*>(TWStringUTF8Bytes(invalidString.get())),
        TWStringSize(invalidString.get())));
