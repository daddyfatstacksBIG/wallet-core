...
TEST(Signer, BitcoinTransactionSign) {
    ...
    auto transaction =
        R"({"hash_type":1,"amount":200000000,"byte_fee":1,"to_address":"1Bp9U1ogV3A14FMvKbRJms7ctyso4Z4Tcx","change_address":"1FQc5LdgGHMHEN9nwkjmz6tWkxhPpxBvBU","scripts":{"4733f37cf4db86fbc2efed2500b4f4e49f312023":")" +
        TW::Base64::encode(parse_hex("001479091972186c449eb1ded22b78e40d009bdf0089")) +
        R"("},"utxo":{"out_point":{"hash":")" +
        (TW::Base64::encode(
            parse_hex("db6b1b20aa0fd7b23880be2ecbd4a98130974cf4748fb66092ac4d3ceb1a5477"))) +
        R"(","index":1,"sequence":)" + std::to_string(UINT32_MAX) + R"(},"script":")" +
    ...
    ASSERT_EQ(output.output(),
              "01000000000101db6b1b20aa0fd7b23880be2ecbd4a98130974cf4748fb66092ac4d3ceb1a5477010000"
              "001716001479091972186c449eb1ded22b78e40d009bdf0089ffffffff0200c2eb0b000000001976a914"
              "769bdff96a02f9135a1d19b749db6a78fe07dc9088ac1e07af2f000000001976a9149e089b6889e032d4"
              "6e3b915a3392edfd616fb1c488ac02473044022009195d870ecc40f54130008e392904e77d32b738c1ad"
              "d19d1d8ebba4edf812e602204f49de6dc60d9a3c3703e1e642942f8834f3a2cd81a6562a34b293942ce4"
              "2f40012103ad1d8e89212f0b92c74d23bb710c00662ad1470198ac48c43f7d6f93a2a2687300000000");
}

TEST(Signer, BitcoinCashTransactionSign) {
    // SignP2SH_P2WPKH used line 238
    auto transaction =
        R"({"hash_type":)" + std::to_string(TWBitcoinSigHashTypeFork | TWBitcoinSigHashTypeAll) +
        ...
        R"("},"utxo":{"out_point":{"hash":")" +
        (TW::Base64::encode(
            parse_hex("e28c2b955293159898e34c6840d99bf4d390e2ee1c6f606939f18ee1e2000d05"))) +
        R"(","index":2,"sequence":)" + std::to_string(UINT32_MAX) + R"(},"script":")" +
    ...
    ASSERT_TRUE(output.success());
    ASSERT_EQ(output.output(),
              "01000000000101e28c2b955293159898e34c6840d99bf4d390e2ee1c6f606939f18ee1e2000d05020000"
              "006b483045022100b70d158b43cbcded60e6977e93f9a84966bc0cec6f2dfd1463d1223a90563f0d0220"
              "7548d081069de570a494d0967ba388ff02641d91cadb060587ead95a98d4e3534121038eab72ec78e639"
              "d02758e7860cdec018b49498c307791f785aa3019622f4ea5bffffffff0258020000000000001976a914"
              "769bdff96a02f9135a1d19b749db6a78fe07dc9088ace5100000000000001976a9149e089b6889e032d4"
              "6e3b915a3392edfd616fb1c488ac0000000000");
}

TEST(Signer, CosmosTransactionSign) {
    auto transaction =
        R"({"accountNumber":"8733","chainId":"cosmoshub-2","fee":{"amounts":[{"denom":"uatom","amount":"5000"}],"gas":"200000"}, "memo":"Testing", "messages":[{"sendCoinsMessage":{"fromAddress":"cosmos1ufwv9ymhqaal6xz47n0jhzm2wf4empfqvjy575","toAddress":"cosmos135qla4294zxarqhhgxsx0sw56yssa3z0f78pm0","amounts":[{"denom":"uatom","amount":"995000"}]}}]})";
    auto input = Proto::SigningInput();
    ...
    ASSERT_TRUE(output.success());
    ASSERT_EQ(
        "{\"mode\":\"block\",\"tx\":{\"fee\":{\"amount\":[{\"amount\":\"5000\",\"denom\":\"uatom\"}"
        "],\"gas\":\"200000\"},\"memo\":\"Testing\",\"msg\":[{\"type\":\"cosmos-sdk/"
        "MsgSend\",\"value\":{\"amount\":[{\"amount\":\"995000\",\"denom\":\"uatom\"}],\"from_"
        "address\":\"cosmos1ufwv9ymhqaal6xz47n0jhzm2wf4empfqvjy575\",\"to_address\":"
        "\"cosmos135qla4294zxarqhhgxsx0sw56yssa3z0f78pm0\"}}],\"signatures\":[{\"pub_key\":{"
        "\"type\":\"tendermint/PubKeySecp256k1\",\"value\":\"A6EsukEXB53GhohQVeDpxtkeH8KQIayd/Co/"
        "ApYRYkTm\"},\"signature\":\"ULEpUqNzoAnYEx2x22F3ANAiPXquAU9+mqLWoAA/"
        "ZOUGTMsdb6vryzsW6AKX2Kqj1pGNdrTcQ58Z09JPyjpgEA==\"}]}}",
        output.output());
}

TEST(Signer, BinanceTransactionSign) {
    auto transaction =
        R"({"chainId":"Binance-Chain-Tigris","accountNumber":"13186","source":"2","memo":"Testing","sendOrder":{"inputs":[{"address":"EuZU7e+eUIuDNzaph9Bp2lqJrts=","coins":[{"denom":"BNB","amount":"1345227"}]}],"outputs":[{"address":"M7vzB7mBRvE9IGk8+UbC13pMryg=","coins":[{"denom":"BNB","amount":"1345227"}]}]}})";
    auto input = Proto::SigningInput();
    ...
    ASSERT_TRUE(output.success());
    ASSERT_EQ("ca01f0625dee0a4a2a2c87fa0a210a1412e654edef9e508b833736a987d069da5a89aedb12090a03424e"
              "4210cb8d5212210a1433bbf307b98146f13d20693cf946c2d77a4caf2812090a03424e4210cb8d52126d"
              "0a26eb5ae9872102e58176f271a9796b4288908be85094a2ac978e25535fd59a37b58626e3a84d9e1240"
              "015b4beb3d6ef366a7a92fd283f66b8f0d8cdb6b152a9189146b27f84f507f047e248517cf691a36ebc2"
              "b7f3b7f64e27585ce1c40f1928d119c31af428efcf3e1882671a0754657374696e672002",
              output.output());
}
...
TEST(Signer, TezosTransactionSign) {
    ...
    ASSERT_EQ(
        ...
        "cc8c90dfa3b0b95721f80992f001f44e810200429a986c8072a40a1f3a3e2ab5a5819bb1b2fb69993c50048378"
        "15b9dc55923e6c0081faa75f741ef614b0e35fcc8c90dfa3b0b95721f80993f001f44e810201000081faa75f74"
        "1ef614b0e35fcc8c90dfa3b0b957210001b86398d5b9be737dca8e4106ea18d70e69b75e92f892fb283546a991"
        "52b8d7794b919c0fbf1c31de386069a60014491c0e7505adef5781cead1cfe6608030b",
        output.output());
}

TEST(Signer, IoTeXTransactionSign) {
    auto transaction =
        R"({"version": 1,"nonce": 123,"gasLimit": 888,"gasPrice": "999","transfer": {"amount": "456","recipient": "io187wzp08vnhjjpkydnr97qlh8kh0dpkkytfam8j","payload": "aGVsbG8gd29ybGQh"}})";
    auto input = Proto::SigningInput();
...
TEST(Signer, WanchainTransactionSign) {
    auto transaction =
        R"({"chainId": "MQ==","toAddress": "0x3535353535353535353535353535353535353535","nonce": "OQ==","gasPrice": "MjAwMDAwMDAwMDA=","gasLimit": "MjEwMDA=","amount": "MTAwMDAwMDAwMDAwMDAwMDAwMA=="})";
    auto input = Proto::SigningInput();
    ...
    ;
    ASSERT_EQ(
        "f88201398b32303030303030303030308532313030309435353535353535353535353535353535353535359331"
        "303030303030303030303030303030303030808185a0f7bca5c00884bcba5c068f507a33559df775e09785fc55"
        "956c9bbe4276259a4ca06679fc4f853b7f224ac8096d15005a3d1070c81a0a13aee8e60a72654273fe5e",
        output.output());
}
...
TEST(Signer, WavesTransactionSign) {
    ...
    auto transaction =
        R"({"timestamp": 1526641218066, "transferMessage": {"amount": 100000000,"asset": "WAVES","fee": 100000000,"fee_asset": "WAVES","to": "3P2uzAzX9XTu1t32GkWw68YFFLwtapWvDds","attachment": "ZmFsYWZlbA=="} })";
    auto input = Proto::SigningInput();
    ...
    ;
    ASSERT_EQ(
        R"({"amount":100000000,"attachment":"4t2Xazb2SX","fee":100000000,"proofs":["4WUvsbgA2EsyCd5C7jHS57SgvPNczXYnxyKSvAzm18vtRhvbjjH9Dq2vUDgqm8h4GcDzzueQnHg3WH4xTyxDJ1Hg"],"recipient":"3P2uzAzX9XTu1t32GkWw68YFFLwtapWvDds","senderPublicKey":"6mA8eQjie53kd4jbZrwL3ZhMBqCX6nzit1k55tR2X7zU","timestamp":1526641218066,"type":4,"version":2})",
        output.output());
}

TEST(Signer, NebulasTransactionSign) {
    auto transaction =
        R"({"fromAddress": "n1V5bB2tbaM3FUiL4eRwpBLgEredS5C2wLY","chainId": "MQ==","nonce": "Nw==","gasPrice": "MTAwMDAwMA==","gasLimit": "MjAwMDAw","toAddress": "n1SAeQRVn33bamxN4ehWUT7JGdxipwn8b17","amount": "MTEwMDAwMDAwMDAwMDAwMDAwMDA=","timestamp": "MTU2MDA1MjkzOA==","payload": ""})";
    auto input = Proto::SigningInput();
    ...
    ASSERT_TRUE(output.success());
    ;
    ASSERT_EQ("17aca84f1c31f2065123ff14d53f10e2c2a8cd6e99f75fb9b11a2bc7bacb14e05c29f2267c700f9f1c5b"
              "9df26bddc5972993c71be651c5c7872683e23cdd80c301",
              output.output());
}
...
TEST(Signer, VeChhainTransactionSign) {
    ...
    ASSERT_TRUE(output.success());
    ;
    ASSERT_EQ("f86a010101dcdb943535353535353535353535353535353535353535843130303080808252088001c0b8"
...
TEST(Signer, NetworkNotSupported) {
    auto transaction =
        R"({"accountNumber":"8733","chainId":"cosmoshub-2","fee":{"amounts":[{"denom":"uatom","amount":"5000"}],"gas":"200000"},"memo":"Testing","sendCoinsMessage":{"fromAddress":"cosmos1ufwv9ymhqaal6xz47n0jhzm2wf4empfqvjy575","toAddress":"cosmos135qla4294zxarqhhgxsx0sw56yssa3z0f78pm0","amounts":[{"denom":"uatom","amount":"995000"}]}})";
    auto input = Proto::SigningInput();
    input.set_private_key("c9b0a273831931aa4a5f8d1a570d5021dda91d3319bd3819becdaabfb7b44e3b");
    input.set_transaction(transaction);
...
TEST(Signer, NanoTransactionSign) {
    auto transaction =
        R"({"link_block":")" +
        TW::Base64::encode(
            parse_hex("491fca2c69a84607d374aaf1f6acd3ce70744c5be0721b5ed394653e85233507")) +
    ...
    ASSERT_TRUE(output.success());
    ASSERT_EQ("d247f6b90383b24e612569c75a12f11242f6e03b4914eadc7d941577dcf54a3a7cb7f0a4aba4246a40d9"
              "ebb5ee1e00b4a0a834ad5a1e7bef24e11f62b95a9e09",
              output.output());
}

TEST(Signer, StellarTransactionSign) {
    auto transaction =
        R"({"amount": 10000000,"fee": 1000, "sequence": 2, "account": "GAE2SZV4VLGBAPRYRFV2VY7YYLYGYIP5I7OU7BSP6DJT7GAZ35OKFDYI", "destination": "GDCYBNRRPIHLHG7X7TKPUPAZ7WVUXCN3VO7WCCK64RIFV5XM5V5K4A52", "memoId": {"id":1234567890}, "passphrase": "Public Global Stellar Network ; September 2015", "operation_type": "1"})";
    auto input = Proto::SigningInput();
...
TEST(Signer, SolanaTransactionSign) {
    auto transaction =
        R"({"transferTransaction": {"recipient": "EN2sCsJ1WDV8UFqsiTXHcUPUxQ4juE71eCknHYYMifkd", "value": 42}, "recentBlockhash": "11111111111111111111111111111111"})";
    auto input = Proto::SigningInput();
