...
TEST(StoredKey, LoadNonexistent) {
    ASSERT_THROW(StoredKey::load(TESTS_ROOT + "/Keystore/Data/nonexistent.json"),
...
TEST(StoredKey, LoadLegacyPrivateKey) {
    ...
    EXPECT_EQ(hex(key.payload.decrypt("testpassword")),
...
TEST(StoredKey, LoadLivepeerKey) {
    ...
    EXPECT_EQ(key.accounts[0].coin(), TWCoinTypeEthereum);
    EXPECT_EQ(hex(key.payload.decrypt("Radchenko")),
...
TEST(StoredKey, LoadPBKDF2Key) {
    ...
    EXPECT_EQ(hex(payload.decrypt("testpassword")),
              "7a28b5ba57c53603b0b07b56bba752f7784bf506fa95edc395f5cf6c7514fe9d");
}
...
TEST(StoredKey, CreateAccounts) {
    ...
    const auto mnemonicData =
        TW::Data(mnemonicPhrase.c_str(), mnemonicPhrase.c_str() + mnemonicPhrase.size());
    ...
    EXPECT_EQ(key.account(TWCoinTypeBitcoin, &wallet)->address,
              "bc1qturc268v0f2srjh4r2zu4t6zk4gdutqd5a6zny");
    EXPECT_EQ(key.account(TWCoinTypeBitcoin, &wallet)->extendedPublicKey,
...
TEST(StoredKey, MissingAddress) {
    ...
    EXPECT_EQ(key.account(TWCoinTypeBitcoin, nullptr)->address,
