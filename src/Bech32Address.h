...
class Bech32Address {
  ...
  public:
    ...
    /// obj_out:  Pass-by-ref, result is initialized here if possible, it can be a derived address
    /// type. hrp: the expected hrp prefix (if missing ("") no prefix check is done). \returns true
    /// if success.
    ...
    Bech32Address(const std::string& hrp, const Data& keyHash)
