...
struct TWZcashTransactionSigner* _Nonnull TWZcashTransactionSignerCreate(
    TW_Bitcoin_Proto_SigningInput data) {
    Bitcoin::Proto::SigningInput input;
    input.ParseFromArray(TWDataBytes(data), static_cast<int>(TWDataSize(data)));
    return new TWZcashTransactionSigner{
        Bitcoin::TransactionSigner<Transaction, TransactionBuilder>(std::move(input))};
}

struct TWZcashTransactionSigner* _Nonnull TWZcashTransactionSignerCreateWithPlan(
    ...
    return new TWZcashTransactionSigner{Bitcoin::TransactionSigner<Transaction, TransactionBuilder>(
        std::move(input), std::move(plan))};
}
...
TWZcashTransactionSignerPlan(struct TWZcashTransactionSigner* _Nonnull signer) {
