...
struct TWStoredKey* _Nonnull TWStoredKeyCreate(TWString* _Nonnull name,
                                               TWString* _Nonnull password) {
    ...
    auto data =
        TW::Data(mnemonic.mnemonic.c_str(), mnemonic.mnemonic.c_str() + mnemonic.mnemonic.size());
    return new TWStoredKey{
        StoredKey(StoredKeyType::mnemonicPhrase, nameString, passwordString, data)};
}
...
struct TWStoredKey* _Nullable TWStoredKeyImportPrivateKey(TWData* _Nonnull privateKey,
    ...
    auto& nameString = *reinterpret_cast<const std::string*>(name);
    auto& passwordString = *reinterpret_cast<const std::string*>(password);
    auto result = new TWStoredKey{
        StoredKey(StoredKeyType::privateKey, nameString, passwordString, privateKeyData)};
...
struct TWStoredKey* _Nullable TWStoredKeyImportHDWallet(TWString* _Nonnull mnemonic,
    ...
    auto result =
        new TWStoredKey{StoredKey(StoredKeyType::mnemonicPhrase, nameString, passwordString, data)};
