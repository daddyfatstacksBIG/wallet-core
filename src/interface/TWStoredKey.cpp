...
struct TWStoredKey* _Nullable TWStoredKeyImportPrivateKey(TWData* _Nonnull privateKey,
    ...
    try {
        ...
        const auto& nameString = *reinterpret_cast<const std::string*>(name);
        const auto passwordData = TW::data(TWDataBytes(password), TWDataSize(password));
        return new TWStoredKey{StoredKey::createWithPrivateKeyAddDefaultAddress(
            nameString, passwordData, coin, privateKeyData)};
    } catch (...) {
        return nullptr;
    }
}
...
struct TWStoredKey* _Nullable TWStoredKeyImportHDWallet(TWString* _Nonnull mnemonic,
    ...
    try {
        ...
        const auto& nameString = *reinterpret_cast<const std::string*>(name);
        const auto passwordData = TW::data(TWDataBytes(password), TWDataSize(password));
        return new TWStoredKey{StoredKey::createWithMnemonicAddDefaultAddress(
            nameString, passwordData, mnemonicString, coin)};
    } catch (...) {
        return nullptr;
    }
}
...
void TWStoredKeyAddAccount(struct TWStoredKey* _Nonnull key, TWString* _Nonnull address,
    ...
    const auto& extetndedPublicKeyString =
        *reinterpret_cast<const std::string*>(extetndedPublicKey);
