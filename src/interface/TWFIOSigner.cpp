        ...
        if (!in.ParseFromArray(TWDataBytes(input), static_cast<int>(TWDataSize(input)))) {
            ...
            if (in.action().has_register_fio_address_message()) {
                ...
                json = TransactionBuilder::createRegisterFioAddress(
                    ...
                    owner.string(), getChainParams(in), action.fee(), action.tpid(), in.expiry());
                ...
                for (int i = 0; i < action.public_addresses_size(); ++i) {
                    addresses.push_back(std::make_pair(action.public_addresses(i).token_code(),
                                                       action.public_addresses(i).address()));
                }
                json = TransactionBuilder::createAddPubAddress(
                    owner, privateKey, action.fio_address(), addresses, getChainParams(in),
                    action.fee(), action.tpid(), in.expiry());
            } else if (in.action().has_transfer_message()) {
                const auto action = in.action().transfer_message();
                json = TransactionBuilder::createTransfer(
                    owner, privateKey, action.payee_public_key(), action.amount(),
