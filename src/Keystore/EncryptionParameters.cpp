...
Data EncryptionParameters::decrypt(const std::string& password) const {
    ...
    if (kdfParams.which() == 0) {
        ...
        scrypt(reinterpret_cast<const byte*>(password.data()), password.size(),
               scryptParams.salt.data(), scryptParams.salt.size(), scryptParams.n, scryptParams.r,
               scryptParams.p, derivedKey.data(), scryptParams.defaultDesiredKeyLength);
        mac = computeMAC(derivedKey.end() - 16, derivedKey.end(), encrypted);
        ...
        pbkdf2_hmac_sha256(reinterpret_cast<const byte*>(password.data()), password.size(),
                           pbkdf2Params.salt.data(), pbkdf2Params.salt.size(),
                           pbkdf2Params.iterations, derivedKey.data(),
