...
public:
    ParamSet() = default;
    ParamSet(const std::shared_ptr<ParamBase>& param1) {
        addParam(param1);
    }
    ParamSet(const std::vector<std::shared_ptr<ParamBase>>& params) {
        addParams(params);
    }
    ~ParamSet();
    ...
    std::vector<std::shared_ptr<ParamBase>> const& getParams() const {
        return _params;
    }
...
public:
    ...
    void addParams(const std::vector<std::shared_ptr<ParamBase>>& params) {
        _params.addParams(params);
    }
    std::shared_ptr<ParamBase> getParam(int paramIndex) const {
        return _params.getParamUnsafe(paramIndex);
    }
    virtual std::string getType() const {
        return _params.getType();
    }
