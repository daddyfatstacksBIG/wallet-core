...
public:
    ...
    ParamByteArray(const Data& val) : ParamCollection() {
        setVal(val);
    }
    void setVal(const Data& val) {
        _bytes = val;
    }
    const Data& getVal() const {
        return _bytes;
    }
    ...
    virtual size_t getSize() const {
        return 32 + ValueEncoder::paddedTo32(_bytes.size());
    }
...
public:
    ...
    virtual std::string getType() const {
        return "bytes" + std::to_string(_n);
    };
    virtual size_t getSize() const {
        return ValueEncoder::paddedTo32(_bytes.size());
    }
    virtual bool isDynamic() const {
        return false;
    }
...
public:
    ParamString() = default;
    ParamString(std::string val): ParamCollection() {
        setVal(val);
    }
    ...
    virtual size_t getSize() const {
        return 32 + ValueEncoder::paddedTo32(_str.size());
    }
    virtual bool isDynamic() const {
        return true;
    }
    ...
    virtual void encode(Data& data) const {
        ParamString::encodeString(_str, data);
    }
