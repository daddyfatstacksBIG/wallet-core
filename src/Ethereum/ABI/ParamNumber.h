...
public:
    ...
    virtual size_t getSize() const {
        return ValueEncoder::encodedIntSize;
    }
    ...
    static bool decodeNumber(const Data& encoded, T& decoded, size_t& offset_inout) {
        uint256_t val256;
        if (!ABI::decode(encoded, val256, offset_inout)) {
            return false;
        }
        // cast down
...
public:
    ...
    uint256_t getVal() const {
        return ParamNumberType<uint256_t>::getVal();
    }
};
...
public:
    ...
    int256_t getVal() const {
        return ParamNumberType<int256_t>::getVal();
    }
};
...
public:
    ...
    bool getVal() const {
        return ParamNumberType<bool>::getVal();
    }
};
...
public:
    const size_t bits;
    ParamUIntN(size_t bits_in) : bits(bits_in) {
        init();
    }
    ParamUIntN(size_t bits_in, uint256_t val) : bits(bits_in) {
        init();
        setVal(val);
    }
    ...
    virtual void encode(Data& data) const {
        ValueEncoder::encodeUInt256(_val, data);
    }
...
public:
    const size_t bits;
    ParamIntN(size_t bits_in) : bits(bits_in) {
        init();
    }
    ParamIntN(size_t bits_in, int256_t val) : bits(bits_in) {
        init();
        setVal(val);
    }
    ...
    virtual void encode(Data& data) const {
        ValueEncoder::encodeUInt256((uint256_t)_val, data);
    }
